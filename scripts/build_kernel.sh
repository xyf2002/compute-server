...
if [ -f "/local/.tsc_done" ] && [ ! -f "/local/.vm_setup_done" ]; then
    ...
    sudo uvt-simplestreams-libvirt sync --source https://cloud-images.ubuntu.com/daily/ release=bionic arch=amd64
    ...
    if ! sudo uvt-kvm create "${VM_NAME}" \
            ...
            --cpu 54 --memory 54096 --password 1997 --disk 20; then
